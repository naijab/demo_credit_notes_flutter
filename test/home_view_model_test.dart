...
void main() {
  test('Home view model can add new payment history', () {
    ...
    final payment = CreditCardInfoPaymentHistory(title: "Food", price: 200);
    viewModel.add(payment);

    expect(viewModel.creditCard.value.paymentHistories?.first, payment);
  ...
  test('Home view model can remove old payment history', () {
    ...
    viewModel.remove(payment);
    expect(viewModel.creditCard.value.paymentHistories?.length, 0);
  });
